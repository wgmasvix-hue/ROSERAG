...
const ROLES = [
  { name: "Admin", perms: ["All permissions"], color: "bg-red-500/15 text-red-400" },
  { name: "Researcher", perms: ["Read", "Write", "Search", "Copilot"], color: "bg-blue-500/15 text-blue-400" },
  { name: "Librarian", perms: ["Read", "Write", "Manage connectors"], color: "bg-purple-500/15 text-purple-400" },
  { name: "Student", perms: ["Read", "Search", "Copilot (limited)"], color: "bg-green-500/15 text-green-400" },
  { name: "Viewer", perms: ["Read only"], color: "bg-slate-800 text-slate-300" },
];
  ...
  return (
    <div className="space-y-6">
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
        <h3 className="font-bold text-white mb-4">Workspace Settings</h3>
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1.5">Default Top-K Results</label>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
        <h3 className="font-bold text-white mb-1">LLM Provider</h3>
        <p className="text-xs text-slate-400 mb-4">Configure which AI provider powers your knowledge system</p>
        <div className="grid grid-cols-2 gap-3 mb-4">
          {["DeepSeek (Default)", "OpenAI", "Anthropic", "Ollama (Local)"].map((p) => (
            <label
              key={p}
              className="flex items-center gap-3 p-3 border border-slate-700 rounded-lg cursor-pointer hover:border-rose-500/60 has-[:checked]:border-rose-500 has-[:checked]:bg-rose-500/15 transition-colors"
            >
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-300 mb-1.5">Chat Model</label>
...
function APIKeysTab() {
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-5">
      ...
      <div>
        <label className="block text-xs font-semibold text-slate-300 mb-1.5">Qdrant Host URL</label>
      ...
      <div>
        <label className="block text-xs font-semibold text-slate-300 mb-1.5">Redis URL</label>
...
function DSpaceConfigPanel() {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 border-l-4 border-l-purple-500 rounded-xl p-5 space-y-4">
      <div className="flex items-center gap-3">
        <span className="text-2xl">📚</span>
        <div>
          <div className="flex items-center gap-2">
            <span className="font-semibold text-white">DSpace 7.x</span>
          ...
          <p className="text-xs text-slate-400">Institutional repository REST API</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div>
          ...
          />
          <p className="text-xs text-slate-500 mt-1">Base URL of your DSpace 7.x instance</p>
        </div>
        <div className="grid sm:grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1.5">Service Account Email</label>
            <input
              className="input"
              type="email"
      ...
      <div className="flex gap-3">
        <button
          ...
          className="bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 rounded-lg flex-1 justify-center text-xs inline-flex items-center gap-2 px-3 py-2 font-medium transition-colors disabled:opacity-50"
        >
          {status === "testing"
      ...
      <p className="text-xs text-slate-500">
        Set <code className="bg-slate-800 px-1 rounded text-slate-300">DSPACE_URL</code>,{" "}
        <code className="bg-slate-800 px-1 rounded text-slate-300">DSPACE_EMAIL</code>, and{" "}
        <code className="bg-slate-800 px-1 rounded text-slate-300">DSPACE_PASSWORD</code> in your{" "}
        <code className="bg-slate-800 px-1 rounded text-slate-300">.env</code> file to persist this configuration.
      </p>
    </div>
  );
}
...
function ConnectorsTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h3 className="font-bold text-white mb-3">Institutional Repository</h3>
        <DSpaceConfigPanel />
      </div>

      {/* Other connectors */}
      <div>
        <h3 className="font-bold text-white mb-3">Other Connectors</h3>
        <div className="grid sm:grid-cols-2 gap-4">
          {CONNECTORS_LIST.filter((c) => c.id !== "dspace").map((c) => (
            <div key={c.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 hover:border-rose-500/30 transition-colors">
              ...
              <button
                ...
                className={configured[c.id]
                  ? "bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 rounded-lg w-full justify-center text-xs inline-flex items-center gap-2 px-3 py-1.5 font-medium transition-colors"
...
function SecurityTab() {
  return (
    <div className="space-y-6">
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
        <h3 className="font-bold text-white mb-4">Role-Based Access Control</h3>
        <div className="space-y-3">
          {ROLES.map((role) => (
            <div key={role.name} className="flex items-center gap-4 p-3 bg-slate-800/50 rounded-lg">
              <span className={`badge ${role.color} min-w-[90px] justify-center`}>{role.name}</span>
              <div className="flex flex-wrap gap-2">
                {role.perms.map((p) => (
                  <span key={p} className="text-xs text-slate-300 bg-slate-800 border border-slate-700 rounded px-2 py-0.5">
                    {p}
                  </span>
                ))}
              </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
        ...
        <div className="space-y-3">
          {[
            ...
            <label key={m.label} className="flex items-center justify-between p-3 bg-slate-800/50 rounded-lg cursor-pointer hover:bg-slate-800 transition-colors">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
        ...
        <p className="text-xs text-slate-400 mb-4">All user actions are logged for compliance</p>
        <div className="space-y-2">
          {[
            ...
            <div key={i} className="flex items-center gap-3 text-xs p-2.5 bg-slate-800/50 rounded-lg">
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-400 text-sm mt-1">Configure your RoseRAG Knowledge Operating System</p>
      </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {["notifications", "team"].includes(activeTab) && (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center">
              ...
              <p className="text-slate-400 text-sm">This section is under active development.</p>
            </div>
          )}
        </div>
      </div>
