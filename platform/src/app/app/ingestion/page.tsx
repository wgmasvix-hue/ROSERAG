  ...
  return (
    <div className="space-y-5">
      ...
      <div
        className={`border-2 border-dashed rounded-2xl p-10 text-center transition-all cursor-pointer
          ${dragging
            ...
            : "border-slate-700 bg-slate-900/50 hover:border-rose-500 hover:bg-rose-500/10"
          }`}
      ...
      >
        ...
        <CloudUpload className={`w-12 h-12 mx-auto mb-3 ${dragging ? "text-rose-400" : "text-slate-500"}`} />
        <p className="font-semibold text-slate-200 mb-1">Drop files here or click to browse</p>
        <p className="text-xs text-slate-500">Max 50 MB per file · Multiple files supported</p>
      ...
      {files.length > 0 && (
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-white text-sm">Upload Queue ({files.length})</h3>
              ...
              <div key={f.id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs text-slate-500 ml-2 flex-shrink-0">{f.size}</span>
                  </div>
                ...
                <button onClick={() => remove(f.id)} className="text-slate-600 hover:text-rose-400 transition-colors flex-shrink-0">
    ...
    <div className="space-y-5">
      <div className="card p-5 space-y-4">
        <h3 className="font-bold text-white text-sm flex items-center gap-2">
          <Globe className="w-4 h-4 text-rose-400" /> Add URL or Website
        </h3>
        <div className="flex gap-2">
          <input
      ...
      {queue.length > 0 && (
        <div className="card p-5">
          <h3 className="font-bold text-white text-sm mb-4">URL Queue ({queue.length})</h3>
          <div className="space-y-3">
            {queue.map((entry) => (
              <div key={entry.id} className="flex items-center gap-3 p-3 rounded-lg bg-slate-800">
                <Globe className="w-4 h-4 text-slate-500 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-slate-300 truncate">{entry.url}</p>
                  {entry.pages && (
                    <p className="text-xs text-slate-500 mt-0.5">{entry.pages} pages ingested</p>
                  )}
                </div>
          ...
          <div key={c.id} className="card p-4 flex flex-col gap-3">
            ...
            {c.docs > 0 && (
              <div className="text-xs text-slate-500">
                <span className="font-semibold text-slate-300">{c.docs.toLocaleString()}</span> documents indexed
              </div>
            )}
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Documents Indexed", value: "12,847", icon: FileText, bg: "bg-blue-500/15", text: "text-blue-400" },
          { label: "Active Connectors", value: "4 / 9", icon: Plug, bg: "bg-green-500/15", text: "text-green-400" },
          { label: "Processing Queue", value: "3 items", icon: Clock, bg: "bg-amber-500/15", text: "text-amber-400" },
        ].map((s) => (
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-white">Recent Ingestion Activity</h3>
          <button className="text-xs text-rose-400 hover:text-rose-300 font-medium flex items-center gap-1 transition-colors">
        ...
        <div className="space-y-3">
          {RECENT_ACTIVITY.map((a) => {
            ...
            const iconColor = a.status === "done" ? "text-green-400" : a.status === "error" ? "text-red-400" : "text-amber-400";
            return (
              <div key={a.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-slate-800 transition-colors">
                <Icon className={`w-4 h-4 mt-0.5 flex-shrink-0 ${iconColor}`} />
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-slate-200 truncate">{a.label}</p>
                  <p className="text-xs text-slate-500 mt-0.5">
                    {a.source} · {a.time}
                    {a.docs > 0 && <> · <span className="font-semibold text-slate-400">{a.docs} docs</span></>}
                  </p>
                </div>
